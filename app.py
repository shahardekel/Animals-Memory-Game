...
#we have to initialize before using any of the pygame functionality
pygame.init()

#we want to control the display
display.set_caption('The Memory Game by Shahar Dekel')
...
#game loop
while running:
    current_events = event.get() #return all mouse and keyboard functions as a list
    ...
    for i, tile in enumerate(tiles):
        ...
        if not tile.skip:
            screen.blit(current_image, (tile.col * gc.IMG_SIZE + gc.MARGIN, tile.row * gc.IMG_SIZE + gc.MARGIN))
        else:
            total_skipped += 1
